...
loss = sys.argv[3] == '1'
assert [r['phase'] for r in rows] == (['baseline', 'rejected', 'done'] if loss else ['baseline', 'cutover', 'rollback', 'done'])
assert all(set(r['fresh']) == {'b1', 'b2'} for r in rows[:-1])
assert [len(r['retained']) for r in rows] == ([0, 4, 8] if loss else [0, 4, 8, 12])
if sys.argv[2] == '1':
    assert all(s['status'] == 'pass' for r in rows for s in r['retained']), rows
    print('SYNC_LOSS_REJECTED' if loss else 'SYNC_CUTOVER_PASS')
