...
if [ "${L4LOAD_BGP_LOAD:-0}" = 1 ]; then
    ...
    if [ "${L4LOAD_BGP_SOAK:-0}" = 1 ]; then
        soak_seconds=${L4LOAD_BGP_SOAK_SECONDS:-60}
        test "$soak_seconds" -ge 60 && test "$soak_seconds" -le 600
...
if [ "${L4LOAD_BGP_SOAK:-0}" = 1 ]; then
    ...
    for protocol in tcp udp; do
        ...
        ip netns exec l4-client sockperf under-load -i 198.18.0.1 -p 8081 --client_ip 10.0.0.2 "${flags[@]}" -m 64 -t "$soak_seconds" --mps 1000 --reply-every 1 --full-rtt --full-log "$out/soak-$protocol.csv" > "$out/soak-$protocol.txt" 2>&1 &
        pids+=("$!")
