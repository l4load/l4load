...
out = Path(sys.argv[1])
expected = int(os.environ.get('L4LOAD_BGP_SOAK_SECONDS', '60'))
summary = {}
for protocol in ('tcp', 'udp'):
    ...
    assert float(duration) >= expected - 1 and int(sent) > 0 and int(received) == int(sent) and dropped == 0 and samples
