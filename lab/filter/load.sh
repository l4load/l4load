...
for repeat in 1 2 3; do
    for rate in 0 100000; do
        ...
        if [ "$rate" != 0 ]; then
            timeout --kill-after=2s 20s ip netns exec l4-client sockperf throughput \
                -i 198.18.0.1 -p 8080 --client_ip 10.0.0.3 -m 64 -t 12 --mps "$rate" \
        ...
        timeout --kill-after=2s 15s ip netns exec l4-client sockperf under-load \
            -i 198.18.0.1 -p 8080 --client_ip 10.0.0.2 -m 64 -t 5 --mps 1000 \
