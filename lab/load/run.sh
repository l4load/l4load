...
for protocol in udp tcp; do
    ...
    for backend in 1 2; do
        ip netns exec "l4-b$backend" sockperf server -i 198.18.0.1 -p 8080 "${flags[@]}" > "$out/server-$protocol-$backend.txt" 2>&1 &
        pids+=("$!")
    done
    sleep 1
    ...
    for repeat in 1 2 3; do
        for rate in 1000 10000; do
            ...
            timeout --kill-after=2s 20s ip netns exec l4-client sockperf under-load \
