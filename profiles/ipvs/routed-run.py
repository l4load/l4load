...
try:
    ...
    for role in ('backup', 'master'):
        ...
        started.append(role)
    gate = subprocess.Popen(['/usr/bin/python3', '/usr/local/libexec/l4load-route-gate.py', str(control), 'vip', config['peer_ip'], '--sync-interface', config['sync_interface'], '--ipvs', '--', f'/etc/l4load/check-{instance}'])
